...
| **P4 — VisitPlan / VisitPlanEntry** | New `visit_plans` (rep + `week_start_date`, unique per rep/week) and `visit_plan_entries` (`visit_plan_id` + `customer_id` + day-only `planned_date`, unique triple, no time — "the calendar should be simple"). `VisitPlan`/`VisitPlanEntry` models (uuid + `HasPublicUuid`, `scopeVisibleTo` mirroring `Prospect`); `VisitPlanController`/`VisitPlanService` with a get-or-create-this-week's-plan endpoint plus add/remove-entry endpoints, gated by `VisitPlanEntryPolicy`. Prospects can never be plan entries; prospect visits stay out of scope per P3. Adds `visits.visit_plan_entry_id` (FK to `visit_plan_entries`, `nullOnDelete`) — no FK-less columns. `VisitPlanEntry` is this codebase's first **soft-deleted** business record (`deleted_by` + a partial-unique index so a removed entry doesn't block re-adding the same customer/day); this became the standing convention for CRM records going forward (see `CLAUDE.md` Conventions). Visit→plan **auto-link is built**: `VisitService::start()` matches rep + customer + today's date against open `VisitPlanEntry` rows and sets `Visit.visit_plan_entry_id` in the same insert, with app-level dedup against double-claiming (ADR 0005; the DB-level unique constraint is deferred, tracked in `docs/backlog.md`) — no manual "tick" UI on web or mobile, as decided. Entries **freeze once `planned_date <= today`**: `VisitPlanService::addEntry`/`removeEntry` and the policy reject same-day-or-past edits server-side (not just a UI disable), protecting the future planned-vs-actual report from being gamed. **Web breaks the P2/P3 view-only pattern on purpose**: the "My Week" planning screen (`views/plans/` — real add/remove via a server-searched customer picker, past/today days rendered read-only) ships on web, per `new__plan.md` putting weekly planning there. **Done 2026-07-14.** The planned-vs-actual coverage report the auto-link and freeze existed for shipped 2026-07-15 as a follow-on: `CoverageReportService`/`ReportController` (`GET /api/reports/coverage/my-week` and `/team`, status derived on read — visited/missed/pending — rather than stored, since nothing writes on the pending→missed transition), split into two permission-gated endpoints (`visits.view` self-only vs. `roles.manage` team-wide) so identity always comes from the session, never a client-supplied rep id, plus a web screen at `/coverage-report`. Close-out done 2026-07-20 (ADR 0006 IDOR split, ADR 0007 computed-vs-stored status, journal, code review). Open: no `VisitPlanSeeder`; an open visit (`ended_at IS NULL`) counts as "visited" (review finding) — both in `docs/backlog.md`. | `visits.*` (reused — a plan entry is a scheduled visit, not a new module) |
